...
def generar_grafico_TISA_TOT(df, img_name="grafico_tisa_tot.png"):
    # --- Preparar DataFrame ---
    df = pd.DataFrame(df)
...
def generar_grafico_tisa_modalidad(df, img_name="grafico_tisa_modalidad.png"):
    # --- Validar DataFrame ---
    df = pd.DataFrame(df)
    if df.empty:
        logging.warning("No hay datos para generar el gráfico TISA por modalidad")
        return None
...
def index_mtc():
    ...
    return render_template('planta_tisa.html', img1=img1, img2=img2)
